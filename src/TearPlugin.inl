...
namespace sofa::component::forcefield
{
    ...
    TearPlugin<DataTypes>::TearPlugin()
        : d_tearThreshold(initData(&d_tearThreshold, "tearThreshold", "")), d_draw(initData(&d_draw, false, "draw", "draw"))
    {
        ...
        if (stepCnt % 10 == 0 && !reachEnd)
        {
            ...
            if (maxStressOfAll > d_tearThreshold.getValue())
            {
                ...
                else
                {
                    ...
                    if (pointSide == 1)
                        edges = trueEdgeAroundPa;
                    else if (pointSide == 2)
                        edges = trueEdgeAroundPb;
                ...
                for (int i = 0; i < 3; i++)
                {
                    if (t1[i] == pointInInciesEdge[0])
                        t1v1 = i;
                    if (t1[i] == pointInInciesEdge[1])
                        t1v2 = i;
                ...
                newTriangle[0][0] = t1[t1v3];
                newTriangle[0][1] = pointInInciesEdge[0];
                newTriangle[0][2] = newPointIndex;
                newTriangle[1][1] = t1[t1v3];
                newTriangle[1][0] = pointInInciesEdge[1];
                newTriangle[1][2] = newPointIndex;
                newTriangle[2][1] = t2[t2v3];
                newTriangle[2][0] = pointInInciesEdge[0];
                newTriangle[2][2] = newPointIndex;
                newTriangle[3][0] = t2[t2v3];
                newTriangle[3][1] = pointInInciesEdge[1];
                newTriangle[3][2] = newPointIndex;
                ...
                if (incieseOk && !reachEnd)
                {
                    ...
                    else if (pointSide == 1)
                    {
                        ...
                        if ((checkPos - NewPos).norm() == 0)
                        {
                            newPointIdx = 0;
                            dmsg_warning() << "is Zero";
                        }
                        else
                            newPointIdx = 1;
